...
class Player {
    ...
    public: 
    int curr_chance, curr_round, total_score, curr_round_score, state, last_round, given_last_chances;
    
    Player() : curr_chance(0), curr_round(0), total_score(0), state(ROUND_PROGRESS), curr_round_score(0), given_last_chances(0) {}
    ~Player() {}
...
int Player :: isStrike(int chance_score) {
    return (curr_chance == FIRST_CHANCE && curr_round_score == ALL_PINS) || (chance_score == 10);
}
...
int Player :: complete_chance(int chance_score) {
    ...
    if(chance_score < 0 || chance_score > 10) {
        cout<<"ERROR: Score cannot be more than 10"<<endl;
        exit(1);
    }
    
    if(isStrike(chance_score)) {
        add_bonus(STRIKE);
        cout<<"! Strike !"<<endl;
...
void Game :: handle_player(int curr_player, int curr_round, int curr_chance, int last_round = 0) {
    ...
    if(last_round_handling) {
        ...
        player.curr_chance = FIRST_CHANCE;
        handle_player(curr_player, curr_round, player.curr_chance++ , 1);
    } 
